...
impl SquirrelDB {
  ...
  /// Connect to SquirrelDB server with auth token
  pub async fn connect_with_auth(addr: &str, token: &str) -> Result<Self> {
    let parts: Vec<&str> = addr.split(':').collect();
    let host = parts.first().unwrap_or(&"localhost");
    let port: u16 = parts.get(1).and_then(|p| p.parse().ok()).unwrap_or(8082);

    Self::connect_with_options(ConnectOptions::new(*host, port).with_auth(token)).await
  }
  ...
  /// Reader loop that processes incoming messages
  ...
  pub async fn query_raw(&self, query: &str) -> Result<serde_json::Value> {
    ...
    match self.request(msg).await? {
      ServerMessage::Result { data, .. } => Ok(data),
      ServerMessage::Error { error, .. } => Err(Error::Server(error)),
      _ => Err(Error::Server("Unexpected response".to_string())),
    }
  }
  ...
  pub async fn subscribe(&self, query: &str) -> Result<Subscription<'_>> {
    ...
    self
      ...
      .write()
      .await
      .insert(id.clone(), ActiveSubscription { tx });
